...
- sincronizzazione dei registri tramite `mutex`;
...
## 29. Limiti attuali e ottimizzazioni future
...
Questa soluzione è semplice e corretta per la configurazione.

Nel percorso di ogni system call potrebbe però introdurre:

- costo lineare;
- contesa sul mutex;
- serializzazione delle letture;
- overhead per l’identificazione ripetuta dell’eseguibile.

Prima dell’intercettazione definitiva sarà necessario valutare:

- hash table;
- strutture read-mostly;
- RCU;
- cache dell’identità;
- separazione tra aggiornamenti rari e consultazioni frequenti.
...
```text
M6 — Registro dei numeri di system call
```

Dovrà comprendere:

- rappresentazione dei numeri x86-64;
- validazione del range;
- aggiunta e rimozione root-only;
- conteggio;
- lista;
- snapshot;
- test di duplicati e valori non validi;
- funzione di matching per il futuro monitor.

Successivamente:

1. configurazione di `MAX`;
2. finestra temporale di un secondo;
3. intercettazione delle system call;
4. combinazione delle condizioni UID o programma;
